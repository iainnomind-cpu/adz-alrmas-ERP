/*
  ...
  - `customers` - Master customer data
    ...
    - `phone` (text)
    - `address` (text)
    - `customer_type` (text) - comercio, casa, banco
    - `communication_tech` (text) - telefono, dual, celular
    - `monitoring_plan` (text) - Plan type
  ...
  - `assets` - Installed equipment tracking
    - `id` (uuid, primary key)
    - `customer_id` (uuid, foreign key)
    - `alarm_model` (text) - DSC 1832, 585, 5010, etc.
    - `keyboard_model` (text)
    - `communicator_model` (text)
    - `serial_number` (text)
    - `installation_date` (date)
    - `is_eol` (boolean) - End of Life status
    - `eol_date` (date)
    - `status` (text) - active, maintenance, obsolete
    - `created_at` (timestamptz)
    - `updated_at` (timestamptz)
  ...
  - `service_orders` - Work orders for technicians
    ...
    - `payment_condition` (text) - contado, credito
    - `notes` (text)
    - `created_at` (timestamptz)
  ...
  - `invoices` - Invoice management
    - `id` (uuid, primary key)
    - `customer_id` (uuid, foreign key)
    - `service_order_id` (uuid, foreign key, nullable)
    - `invoice_number` (text, unique)
    - `invoice_type` (text) - service, recurring, upgrade
    - `payment_type` (text) - contado, credito
    - `amount` (decimal)
    - `tax_amount` (decimal)
  ...
  - `dunning_actions` - Automated dunning process
    ...
    - `customer_id` (uuid, foreign key)
    - `invoice_id` (uuid, foreign key)
    - `action_type` (text) - notification, warning, suspension
    - `days_overdue` (integer)
    - `message_sent` (text)
  ...
  - `opportunities` - Sales leads from EOL assets
    ...
    - `customer_id` (uuid, foreign key)
    - `asset_id` (uuid, foreign key)
    - `opportunity_type` (text) - upgrade, replacement, expansion
    - `estimated_value` (decimal)
    - `status` (text) - open, contacted, quoted, won, lost
