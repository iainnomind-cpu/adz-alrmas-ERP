      ...
      <div className="space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button
            ...
            className="w-full md:w-auto px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all flex items-center justify-center gap-2 font-medium shadow-sm hover:shadow-md whitespace-nowrap"
          >
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 mb-4">
            ...
            <div className="flex gap-2">
              <div className="relative flex-1 md:flex-none">
                ...
                {showQuickFilters && (
                  <div className="absolute right-0 mt-2 w-72 max-w-[calc(100vw-2rem)] bg-white border border-gray-200 rounded-lg shadow-lg z-10 origin-top-right">
              ...
              <button className="flex-none p-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0">
                {(['all', 'requested', 'assigned', 'in_progress', 'paused', 'completed'] as const).map((status) => (
                  <button
                    ...
                    className={`flex-none px-4 py-2 rounded-lg font-medium whitespace-nowrap transition-all ${filter === status
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0">
                {(['all', 'critical', 'urgent', 'high', 'medium', 'low'] as const).map((priority) => (
                  <button
                    ...
                    className={`flex-none px-4 py-2 rounded-lg font-medium whitespace-nowrap transition-all ${priorityFilter === priority
                      ? 'bg-blue-600 text-white'
                      : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                      }`}
                  >
              ...
              <div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0">
                  <button
                    ...
                    className={`flex-none px-4 py-2 rounded-lg font-medium whitespace-nowrap transition-all ${seriesFilter === 'all'
                      ? 'bg-blue-600 text-white'
                      : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                      }`}
                  >
                  ...
                  {availableSeries.map((series) => (
                    <button
                      ...
                      className={`flex-none px-4 py-2 rounded-lg font-medium whitespace-nowrap transition-all ${seriesFilter === series.id
                        ? 'bg-blue-600 text-white'
                        : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                        }`}
                    >
        ...
        {filteredOrders.length === 0 ? (
          ...
          <div className="grid gap-4 w-full">
            {filteredOrders.map((order) => (
              <div
                key={order.id}
                className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-5 hover:shadow-md transition-all group cursor-pointer w-full overflow-hidden"
                onClick={() => setSelectedOrderId(order.id)}
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-3">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    <div className={`flex-none p-2 rounded-lg ${getStatusColor(order.status).replace('text-', 'text-').replace('bg-', 'bg-')}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {order.folio_series?.series_code && (
                          <span className={`flex-none px-2 py-1 text-xs font-semibold rounded ${searchTerm && order.folio_series.series_code.toLowerCase().includes(searchTerm.toLowerCase())
                  ...
                  <div className="flex flex-row sm:flex-col items-center sm:items-end gap-2 sm:gap-1 flex-none w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 border-t border-gray-100 sm:border-0">
                    <div className="flex gap-2 flex-1 sm:flex-none justify-start sm:justify-end">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold whitespace-nowrap ${getPriorityColor(order.priority)}`}>
                        {getPriorityLabel(order.priority)}
                      </span>
                      <span className={`px-2 py-1 rounded-lg text-xs font-medium whitespace-nowrap ${getStatusColor(order.status)}`}>
                        {getStatusLabel(order.status)}
                      </span>
                    </div>
                    <div className="hidden sm:block opacity-0 group-hover:opacity-100 transition-opacity ml-2 mt-2">
                      <Eye className="w-5 h-5 text-blue-600" />
                    </div>
                  </div>
                </div>

                <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 w-full overflow-hidden">
                  {order.customers?.address && (
                    <div className="flex items-center gap-1.5 min-w-0 overflow-hidden">
                      <MapPin className="w-3.5 h-3.5 flex-none" />
                      <span className="truncate">{order.customers.address}</span>
                    </div>
                  )}
                  {order.customers?.phone && (
                    <div className="flex items-center gap-1.5 flex-none">
                      <span className="font-medium whitespace-nowrap">{order.customers.phone}</span>
                    </div>
                  )}
                  {order.assets?.alarm_model && (
                    <div className="flex items-center gap-1.5 flex-none min-w-0">
                      <Wrench className="w-3.5 h-3.5 flex-none" />
                      <span className="truncate">{order.assets.alarm_model}</span>
                    </div>
                  )}
                  {order.created_at && (
                    <div className="flex items-center gap-1.5 flex-none">
